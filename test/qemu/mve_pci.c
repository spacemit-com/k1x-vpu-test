...
static const MemoryRegionOps mve_mmio_ops = {
	.read                    = mve_mmioread,
	.write                   = mve_mmiowrite,
...
static const MemoryRegionOps mve_io_ops = {
	.read                    = mve_ioread,
	.write                   = mve_iowrite,
	.endianness              = DEVICE_NATIVE_ENDIAN,
	.valid                   = {
...
static void mve_pci_realize(PCIDevice *pci_dev, Error **errp)
{
	...
	ret = mve_emul_construct(&mve->state, mve_pci_irq, mve,
				 mve->gdb_enable);
	if (ret != 0) {
	...
	memory_region_init_io(&mve->mmio, OBJECT(mve), &mve_mmio_ops,
			      &mve->state, "mve_mmio", MVE_MMIO_SIZE);
...
static void mve_class_init(ObjectClass *klass,
			   void *data)
{
...
static void mve_register_types(void)
{
	type_register_static(&mve_info);
}
