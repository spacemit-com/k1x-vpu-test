...
static unsigned int get_attr(uint32_t pte)
{
	return (pte >> 30) & 0xf;
}

static unsigned int get_access(uint32_t pte)
{
	return pte & 0xf;
}
...
static unsigned int get_pte_offset(uint32_t va,
				   unsigned level)
{
	return (va >> ((1 - level) * 10 + 12)) & ((1 << 10) - 1);
}

static unsigned int get_page_offset(uint32_t va)
{
	return va & 0xfff;
}

static uint32_t set_pte(hwaddr addr,
			unsigned attr,
			unsigned access)
{
	return ((attr & 0x3) << 30) |
...
static hwaddr mve_ptw(hwaddr l0,
		      uint32_t va,
		      bool assert_page_fault)
{
	uint32_t pte;
	unsigned o0, o1;
	hwaddr l1, l2;
...
static hwaddr mve_tlb_lookup(t_mve_memory_map *mmap,
			     uint32_t addr)
{
...
void *rascemul_translate_rasc_mem(t_mve_memory_map *mmap,
				  uint32_t addr,
				  unsigned int size,
				  address_callback f)
{
	...
	while (p < end) {
		...
		if (pa != 0)
			cpu_physical_memory_read(pa, p, sizeof(*p));
...
uint32_t mve_memory_read8(t_mve_memory_map *mmap,
			  bool is_axi,
			  uint32_t addr,
			  address_callback f)
{
...
void mve_memory_write8(t_mve_memory_map *mmap,
		       uint32_t addr,
		       uint8_t val,
		       address_callback f)
{
...
void mve_memory_log_axi_access(t_mve_memory_map *mm,
			       t_mve_core *core,
			       t_axi_cookie cookie,
			       uint32_t addr,
			       size_t len,
...
static void mve_irq(void *ctx)
{
	...
	if (mve->irq_cb != NULL)
		mve->irq_cb(mve->irq_arg);
}

static void mve_log(void *ctx,
		    const char *str)
{
	printf("%s: %s\n", __FUNCTION__, str);
}

static void mve_axilog(void *ctx,
		       uint32_t cookie,
		       uint64_t cycle,
		       bool write,
		       uint64_t addr,
...
uint64_t mve_ioread(void *opaque,
		    hwaddr addr,
		    unsigned size)
{
	printf("%s\n", __FUNCTION__);
...
uint64_t mve_mmioread(void *opaque,
		      hwaddr addr,
		      unsigned size)
{
...
void mve_mmiowrite(void *opaque,
		   hwaddr addr,
		   uint64_t value,
		   unsigned size)
{
	...
	if (addr >= lsid_offset) {
		...
		case 0x4: {
			...
			if (state->lsid_to_id[lsid] != MVE_UNMAPPED) {
				mveemul_remove_memory_map(
					state->lsid_to_id[lsid]);
				state->lsid_to_id[lsid] = MVE_UNMAPPED;
			}

			state->tmp_l0 = l0;
			ret = mveemul_allocate_memory_map(
				&state->lsid_to_id[lsid]);
...
int mve_emul_construct(MVEEmulState *state,
		       mve_irq_t irq_cb,
		       void *irq_arg,
		       bool gdb_enable)
{
	...
	for (i = 0; i < MVE_LSID_MAX; i++)
		state->lsid_to_id[i] = MVE_UNMAPPED;
