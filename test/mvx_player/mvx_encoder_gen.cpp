...
int main(int argc, const char *argv[]) {
  ...
  mvx_argp_add_opt(&argp, 'c', "count", true, 1, "100",
                   "Number of frames to generate.");
  mvx_argp_add_opt(&argp, 'x', "exec", true, 1, "1",
                   "Change settings while encoding is ongoing. The main "
                   "purpose is to exercise code for coverage.");
  mvx_argp_add_opt(&argp, 'v', "fps", true, 1, "24", "Frame rate.");
  mvx_argp_add_opt(&argp, 'r', "bps", true, 1, "2000000",
                   "Bit rate. If it is combined with -x then  the value will "
                   "later be 1000 and rate control deactivated.");
  ...
  mvx_argp_add_opt(&argp, 'g', "gop", true, 1, "1",
  ...
  mvx_argp_add_opt(&argp, 'k', "tileCR", true, 1, "2",
                   "Set tile columns and rows");
  mvx_argp_add_opt(&argp, 'y', "bandwidth", true, 1, "10000000",
                   "Set bandwidth limit");
  ...
  mvx_argp_add_opt(&argp, 0, "cip", true, 1, "0",
                   "Constrained Intraprediction, 1 is on, 0 is off");
  mvx_argp_add_opt(&argp, 0, "es", true, 1, "0",
                   "Entropy Sync 1 is on, 0 is off");
  mvx_argp_add_opt(&argp, 0, "tmvp", true, 1, "0", "TMVP 1 is on, 0 is off");
  mvx_argp_add_opt(&argp, 0, "sesc", true, 1, "16",
                   "Stream escaping 1 is on, 0 is off");
  mvx_argp_add_opt(&argp, 0, "hmvsr", true, 1, "16",
                   "Horisontal MV Search Range");
  mvx_argp_add_opt(&argp, 0, "vmvsr", true, 1, "16",
                   "Vertical MV Search Range");
  mvx_argp_add_opt(&argp, 'z', "block", true, 0, "0",
                   "Use video device in blocking mode");
  ...
  InputFrame inputFrame =
      InputFrame(inputFormat, mvx_argp_get_int(&argp, "width", 0),
  ...
  Encoder encoder(mvx_argp_get(&argp, "dev", 0), inputFrame, *outputFile,
                  nonblock);
  ...
  if (mvx_argp_is_set(&argp, "chroma")) {
    encoder.setH264ForceChroma(mvx_argp_get_int(&argp, "chroma", 0));
  }
