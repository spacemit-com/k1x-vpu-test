...
int main(int argc, const char *argv[])
{
    ...
    mvx_argp_add_opt(&argp, 'u', "nalu", true, 1, "0", "Nalu format, START_CODES (0) and ONE_NALU_PER_BUFFER (1), ONE_BYTE_LENGTH_FIELD (2), TWO_BYTE_LENGTH_FIELD (3), FOUR_BYTE_LENGTH_FIELD (3).");
    ...
    mvx_argp_add_opt(&argp, 0, "preload", true, 0, "0", "preload the input stream to memory. the size for input file should be less than 15MBytes.");
    ...
    if (ret == 0) {
        if (md5_filename != NULL && md5ref_filename != NULL) {
            ...
            if (is_md5_ok) {
                printf("-----[Test Result] MVX Decode MD5 Check PASS. Average Framerate: %.2f. \n", fps);
            } else {
                printf("-----[Test Result] MVX Decode MD5 Check FAIL. Average Framerate: %.2f. \n", fps);
                ret = 1;
            }
        } else {
