...
IO::IO(uint32_t format, bool preload, size_t width, size_t height, size_t strideAlign) :
    format(format),
    width(width),
    height(height),
...
Input::Input(uint32_t format, bool preload, size_t width, size_t height, size_t strideAlign) :
...
InputFile::InputFile(istream &input, uint32_t format, size_t width, size_t height, size_t strideAlign, bool preload) :
    Input(format, preload, width, height, strideAlign),
    input(input)
{
    state = 7;
    offset=0;
...
void InputFile::prepare(Buffer &buf)
{
    ...
    if (getNaluFormat() == V4L2_OPT_NALU_FORMAT_ONE_NALU_PER_BUFFER) {
        ...
        for (;;i++) {
            ...
            if(i == curlen) {
                memcpy(static_cast<char *>(iov[0].iov_base)+iovoff,inputBuf+offset,curlen-offset);
                ...
                memcpy(static_cast<char *>(iov[0].iov_base)+iovoff,inputBuf+offset,i-offset);
                iovoff += i - offset;
                ...
                if(input.peek() !=EOF){
                    ...
                    memcpy(static_cast<char *>(iov[0].iov_base)+iovoff, inputBuf, remainlen);
                    iovoff += remainlen;
                    iseof = true;
                    break;
                }
            } else if (0 == memcmp(inputBuf+i, startCode, 4)) {//FIX ME: combine start code
                memcpy(static_cast<char *>(iov[0].iov_base)+iovoff,inputBuf+offset,i-offset);
        ...
        while (!found_frame && !iseof) {
            ...
            result rtn = reader->find_one_frame(slice_cnt, frame_start_pos, frame_size);
            switch (rtn) {
                case reader::RR_OK:
        ...
        for (size_t i = 0; i < iov.size(); ++i) {
            if (!getPreload()) {
                ...
                int read_len = readBuffer(static_cast<char *>(iov[i].iov_base), iov[i].iov_len);
...
int InputFile::readBuffer(char* dest, int len)
{
    ...
    if (len <= remain_len) {
        ...
        //printf("readBuffer. 2. read_pos: %d, total_len: %d, len: %d, remain_len: %d\n", read_pos, total_len, len, remain_len);
    }
    return read_len;
}
...
InputAFBC::InputAFBC(istream &input, uint32_t format, size_t width, size_t height, bool preload) :
...
void InputAFBC::prepare(Buffer &buf)
{
    ...
    if (!getPreload()) {
        ...
        read_len = readBuffer(reinterpret_cast<char *>(iov[0].iov_base), header.frameSize);
    }
...
InputFileFrame::InputFileFrame(istream &input, uint32_t format, size_t width, size_t height,
                               size_t strideAlign, bool preload) :
...
void InputFileFrameWithROI::load_roi_cfg()
{
    ...
    while (roi_is.getline(cfg_file_line_buf, CFG_FILE_LINE_SIZE)){
        ...
        if (3 == sscanf(cfg_file_line_buf, "pic=%d qp=%d num_roi=%d",&pic_index,&qp,&num_roi)) {
            roi.pic_index = pic_index;
            roi.qp = qp;
            roi.num_roi = num_roi;
            roi.roi_present  = true;
            roi.qp_present = true;
        } else if (3 == sscanf(cfg_file_line_buf, "pic=%d num_roi=%d qp=%d",&pic_index,&num_roi,&qp)) {
            roi.pic_index = pic_index;
            roi.qp = qp;
            roi.num_roi = num_roi;
            roi.roi_present  = true;
            roi.qp_present = true;
        } else if (2 == sscanf(cfg_file_line_buf, "pic=%d num_roi=%d qp=%d",&pic_index,&num_roi,&qp)) {
            roi.pic_index = pic_index;
            roi.qp = 0;
            roi.num_roi = num_roi;
            roi.roi_present  = true;
            roi.qp_present = false;
        } else if (2 == sscanf(cfg_file_line_buf, "pic=%d qp=%d num_roi=%d",&pic_index,&qp,&num_roi)) {
            roi.pic_index = pic_index;
            roi.qp = qp;
            roi.num_roi = 0;
...
InputFileFrameWithEPR::InputFileFrameWithEPR(std :: istream & input, uint32_t format, size_t width,
                                size_t height, size_t strideAlign, std :: istream & epr, uint32_t oformat) :
        InputFileFrame(input, format, width, height, strideAlign),
        epr_is(epr),
        outformat(oformat)
{
                ...
                if (outformat != V4L2_PIX_FMT_HEVC) {
                    ...
                    for (int row = 0; row < n_rows; row++)
                    {
                        for (int k=0; k<n_cols; k++) {
                            ...
                            uint8_t force = iter->bc_row_body.uncomp->bpr[row * n_cols + k].force;
                            // top left 64*64
                            erp_adjust_bpr_to_64_64(uncomp_body, top_left, 4*row*max_cols + 4*k, max_cols, force);
                            // top right 64*64
                            erp_adjust_bpr_to_64_64(uncomp_body, top_right, 4*row*max_cols + 4*k + 2, max_cols, force);
                            // bottom left 64*64
                            erp_adjust_bpr_to_64_64(uncomp_body, bot_left, 4*row*max_cols + 2*max_cols + 4*k, max_cols, force);
                            // bottom right 64*64
                            erp_adjust_bpr_to_64_64(uncomp_body, bot_right, 4*row*max_cols + 2*max_cols + 4*k + 2, max_cols, force);
                        }
...
void InputFileFrameWithEPR::read_row_cfg(char * buf, int row, int len, epr_config & config)
{
    ...
    for (int i = 0; i< len; i++) {
        ...
        if (5 != sscanf(sub_buf2, " bpr={%d,%d,%d,%d,%hhi}",
                         &quad_qp_delta[0],
                         &quad_qp_delta[1],
                         &quad_qp_delta[2],
                         &quad_qp_delta[3],
...
OutputFileWithMD5::OutputFileWithMD5(std::ofstream &output, uint32_t format, std::ofstream &output_md5, std::ifstream* md5ref) :
    OutputFile(output, format),
...
bool OutputFileWithMD5::getMd5CheckResult()
{
    return md5_check_result;
}
    ...
    if (V4L2_TYPE_IS_MULTIPLANAR(buf.type))
    {
        for (unsigned int i = 0; i < buf.length; ++i)
        {
            ...
            if (buf.memory == V4L2_MEMORY_MMAP || buf.memory == V4L2_MEMORY_USERPTR) {
                iov = { .iov_base = static_cast<char *>(ptr[i]) + p.data_offset, .iov_len = p.bytesused - p.data_offset };
            } else if (buf.memory == V4L2_MEMORY_DMABUF) {
                iov = { .iov_base = static_cast<char *>(ptr[0]) + p.data_offset, .iov_len = p.bytesused - p.data_offset };
            }
...
void Buffer::setBytesUsed(vector<iovec> &iov)
{
    if (V4L2_TYPE_IS_MULTIPLANAR(buf.type))
    {
        ...
        {
            throw Exception("iovec vector size is larger than V4L2 buffer number of planes. size=%zu, planes=%u",
                            iov.size(), buf.length);
        }
        ...
        for (i = 0; i < iov.size(); ++i)
        {
            ...
            if (buf.memory == V4L2_MEMORY_DMABUF) {
                ...
                if (buf.type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE && iov[i].iov_len != 0) {
...
void Buffer::memoryMap(int fd)
{
    if (V4L2_TYPE_IS_MULTIPLANAR(buf.type))
    {
        for (uint32_t i = 0; i < buf.length; ++i)
        {
            ...
            if (p.length > 0)
            {
                if (buf.memory == V4L2_MEMORY_MMAP) {
                    ...
                    ptr[i] = mmap(NULL, p.length, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
                }
                if (ptr[i] == MAP_FAILED)
                {
        ...
        if (buf.memory == V4L2_MEMORY_DMABUF) {
            ...
            dma_fd = DmabufHeapAllocSystem(bufferAllocator, cpu_access_need, total_length, 0, 0);
            ptr[0] = mmap(NULL, total_length, PROT_READ | PROT_WRITE, MAP_SHARED, dma_fd, 0);
            plane_offset[0] = 0;
    ...
    else
    {
        if (buf.length > 0)
        {
            if (buf.memory == V4L2_MEMORY_MMAP) {
                ...
                ptr[0] = mmap(NULL, buf.length, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
            }
        }
    }
}
...
void Codec::Port::getTrySetFormat()
{
    ...
    if (V4L2_TYPE_IS_MULTIPLANAR(type))
    {
        ...
        if (V4L2_TYPE_IS_OUTPUT(type) && (f.pixelformat == V4L2_PIX_FMT_YUV420_AFBC_8 || f.pixelformat == V4L2_PIX_FMT_YUV420_AFBC_10
                    || f.pixelformat == V4L2_PIX_FMT_YUV422_AFBC_8 || f.pixelformat == V4L2_PIX_FMT_YUV422_AFBC_10)) {
            f.plane_fmt[0].sizeimage = io->getFrameSize();
            log << "getTrySetFormat. set size image to " << io->getFrameSize() << endl;
        }
    }
    else
    {
...
void Codec::Port::controlFramerate()
{
    ...
    if (lastTimestamp == 0) {
        ...
        //printf("---------queue buffer[%d]. last time: %lu us, cur time: %lu us, diffTime: %lu us, remainTime: %lu us, intervalTime: %lu us--------------\n", frames_processed, lastTimestamp/1000l, curTimestamp/1000l, diffTime, remainTime, intervalTime);
...
void Codec::Port::queueBuffer(Buffer &buf)
{
    ...
    if (io->getDir() == 0 && frames_count > 0 && frames_processed >= frames_count - 1 && !buf.isGeneralBuffer()) {
    ...
    if (io->getDir() == 0 && V4L2_TYPE_IS_MULTIPLANAR(b.type) &&
            !buf.isGeneralBuffer()) {
        frames_processed++;
        //printf("-------------queueBuffer yuv frames: %d, frames_count: %d-------------\n", frames_processed, frames_count);
    }
...
void Codec::Port::notifySourceChange() {
    isSourceChange = true;
}
...
void Codec::runPoll()
{
    ...
    while (!eos)
    {
        ...
        if (p.revents & POLLIN)
        {
            ...
            if (timestart_us == 0) {
                if (input.type == V4L2_BUF_TYPE_VIDEO_OUTPUT && getOutputFramesProcessed() >= 1) {
                    gettimeofday(&timestart, NULL);
                    timestart_us = timestart.tv_sec * 1000000ll + timestart.tv_usec;
                    printf("-----Decoder. set timestart_us: %lu us---------\n", timestart_us);
                } else if (input.type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE && getInputFramesProcessed() >= 1) {
                    gettimeofday(&timestart, NULL);
                    timestart_us = timestart.tv_sec * 1000000ll + timestart.tv_usec;
    ...
    if (input.type == V4L2_BUF_TYPE_VIDEO_OUTPUT)
    {
        ...
        avgfps = (((double)(frames_processed*1000.0*1000.0))/(double)(timeend_us - timestart_us));
        printf("-----[Test Result] MVX Decode Done. frames_processed: %lu, cost time: %lu us.\n", frames_processed, timeend_us - timestart_us);
    }
    else if (input.type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE)
    {
        frames_processed = getInputFramesProcessed()-1;
        avgfps = (((double)(frames_processed*1000.0*1000.0))/(double)(timeend_us - timestart_us));
        printf("-----[Test Result] MVX Encode Done. frames_processed: %lu, cost time: %lu us.\n", frames_processed, timeend_us-timestart_us);
    }
}
...
void Codec::checkOutputTimestamp(uint64_t timestamp)
{
    ...
    if (it != timestampList.end()) {
        ...
        cerr << "Incorrect timestamp: " << timestamp << ", don't find it in input timestamp list." << endl;
    }
}
...
bool Codec::Port::handleBuffer()
{
    ...
    /* input source change. we should only handle this on decode output:V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE*/
...
size_t Codec::getSize(uint32_t format, size_t width, size_t height,
                      size_t strideAlign, size_t &nplanes, size_t stride[3], size_t size[3])
{
    size_t s[3][2];
    size_t frameSize = 0;
...
void Decoder::setFWTimeout(int timeout)
{
    output.setFWTimeout(timeout);
}

void Decoder::setProfiling(int enable)
{
    output.setProfiling(enable);
}
...
void Encoder::setH264Bitdepth(uint32_t bd)
{
    output.setH264EncBitdepth(bd);
}
...
void Encoder::setH264FixedQPI(uint32_t fqp)
{
    output.setH264EncFixedQPI(fqp);
}

void Encoder::setH264FixedQPP(uint32_t fqp)
{
    output.setH264EncFixedQPP(fqp);
}

void Encoder::setH264FixedQPB(uint32_t fqp)
{
    output.setH264EncFixedQPB(fqp);
}
...
void Encoder::setFWTimeout(int timeout){
    input.setFWTimeout(timeout);
}

void Encoder::setProfiling(int enable){
    input.setProfiling(enable);
}
