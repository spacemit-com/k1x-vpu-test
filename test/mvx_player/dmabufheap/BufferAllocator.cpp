...
int BufferAllocator::OpenDmabufHeap(const std::string& heap_name) {
  ...
  // cout << "Using DMA-BUF heap named: " << heap_name << ", fd: " << fd <<
  // endl;
...
BufferAllocator::~BufferAllocator() { CloseDmabufHeap(); }
...
int BufferAllocator::DmabufAlloc(const std::string& heap_name, size_t len) {
  ...
  timeCostms = ((timeEnd.tv_sec * 1000 + timeEnd.tv_nsec / 1000000) -
                (timeStart.tv_sec * 1000 + timeStart.tv_nsec / 1000000));
  if (timeCostms > 3000)
    cout << "Alloc " << len << "B dmabuf from " << heap_name << " cost "
         << timeCostms << "ms" << endl;
...
int BufferAllocator::Alloc(const std::string& heap_name, size_t len,
  ...
  if (fd < 0) cout << "Alloc dma buf fail. len is " << len << endl;
