...
class BufferAllocator {
 public:
  ...
  /* *
   * Returns a dmabuf fd if the allocation in system heap(cached/uncached) is
   * successful and an error code otherwise. Allocates in the 'system' heap if
   * CPU access of the buffer is expected and 'system-uncached' otherwise. If
   * the 'system-uncached' heap is not supported, falls back to the 'system'
   * heap. For vendor defined heaps with a legacy ION interface(no heap query
   * support), MapNameToIonMask() must be called prior to invocation of
   * AllocSystem() to map names 'system'(and optionally 'system-uncached' if
   * applicable) to an equivalent heap mask and heap flag configuration;
   * configuration.
   ...
   * For a legacy ion interface, syncs a shared dmabuf fd with memory either
   * using ION_IOC_SYNC ioctl or using callback @legacy_ion_cpu_sync if
   ...
   * function is supplied, this is passed as the second argument to
   * legacy_ion_cpu_sync.
   ...
   * to sync dmabuf_fd with memory. The paremeter will be ignored if the
   * interface being used is not legacy ion.
   * @legacy_ion_custom_data: When the legacy version of ion is in use and a
   * callback function is supplied, this pointer is passed as the third argument
   * to legacy_ion_cpu_sync. It is intended to point to data for performing the
   * callback.
   *
   ...
   * specified. For non-legacy ION and dmabuf heap interfaces,
   * DMA_BUF_IOCTL_SYNC is used.
   * @dmabuf_fd: dmabuf fd. When the legacy version of ion is in use and a
   * callback function is supplied, this is passed as the second argument to
   * legacy_ion_cpu_sync.
   * @sync_type: specifies if sync_type is for read, write or read/write.
   * @legacy_ion_cpu_sync: optional callback for legacy ion interfaces. If
   * specified, will be invoked instead of ion_sync_fd with a dup of ion_fd_ as
   * its argument. The parameter will be ignored if the interface being used is
   * not legacy ion.
   * @legacy_ion_custom_data: When the legacy version of ion is in use and a
   * callback function is supplied, this pointer is passed as the third argument
   * to legacy_ion_cpu_sync. It is intended to point to data for performing the
