...
public:
	...
	public:
		...
		packet_info(uint8_t *buf, uint32_t buf_size) : buf(buf), buf_bytes(buf_size)
		{
			prefix_size=0;
			codec_config=false;
		}
    ...
    bool parse(reader::packet_info packet, info& inf)
    {
        ...
        switch(nal_unit_type)
        {
            ...
            case 8: //PPS
            {
                ...
                //printf("PPS pic_parameter_set_id %d, pic_parameter_set_id %d\n",pic_parameter_set_id,pic_parameter_set_id);
                break;
            }
            case 7: //SPS
            {
...
public:
	...
	void get_remaining_bitstream_data(uint8_t* &remaining_data, uint32_t &remaining_size)
	{
		remaining_data = bitstream_buf + bitstream_pos;
		remaining_size = bitstream_buf_size - bitstream_pos;
	}
	
	void set_eos(bool is_eos)
	{
		this->is_eos = is_eos;
	}
	
	bool is_parser_valid()
	{
		return (NULL != dec);
	}
	...
	bool seek_prefix_in_buffer(uint8_t* buffer, uint32_t buffer_size, uint32_t &found_pos, uint32_t &check)
	{
	...
	result find_one_frame(uint32_t &slice_cnt, uint32_t &frame_start_pos, uint32_t &frame_size)
	{
		result rtn = RR_ERROR;
		bool ret = false;
		uint32_t prefix = ~0u;
		...
		while(1){
			...
			if(info.new_frame || (1 == new_frame_cnt && !info.slice)){//the (!info.slice) case: Frame,Frame,Frame,SPS,PPS,SEI ...
				new_frame_cnt++;
			}
			...
			if(info.config || (!info.slice)){ //the (!info.config && !info.slice) case : SEI
				...
				//printf("%s, frame_size=%d, bitstream_pos=%d.\n", info.config ? "config" : "other", frame_size, bitstream_pos);
				break;
			}
	...
	bool find_all_frames(uint32_t &find_frames, uint32_t &start_pos, uint32_t &all_frames_size)
	{
		result rtn = RR_ERROR;
		...
		while(seeking){
			...
			case RR_EOP_FRAME:
				...
				if(RR_EOP_FRAME == rtn){
					find_frames++;
					//printf("frame_cnt=%d, find_frames=%d, slice_cnt=%d, (%d..+%dB).\n", frame_cnt, find_frames, slice_cnt, frame_start_pos, frame_size);
					printf("frame_cnt=%d, slice_cnt=%d, (%d..+%dB).\n", frame_cnt, slice_cnt, frame_start_pos, frame_size);
