...
int main(int argc, const char *argv[])
{
    ...
    mvx_argp_add_opt(&argp, 0, "gop", true, 1, "0", "GOP: 0 is None, 1 is Bidi, 2 is Low delay, 3 is Pyramid");
    ...
    mvx_argp_add_opt(&argp, 0, "qpi", true, 1, "20", "H264 fixed QP for I frames.");
    mvx_argp_add_opt(&argp, 0, "qpb", true, 1, "20", "H264 fixed QP for B frames.");
    mvx_argp_add_opt(&argp, 0, "qpp", true, 1, "20", "H264 fixed QP for P frames.");
    ...
    mvx_argp_add_opt(&argp, 0, "ltr_period", true, 1, "0", "encoder long term reference period, range from 2 to 254 (inclusive)");
    ...
    mvx_argp_add_opt(&argp, 0, "preload", true, 0, "0", "preload the first 5 yuv frames to memory.");
    mvx_argp_add_opt(&argp, 0, "fw_timeout", true, 1, "5", "timeout value[secs] for watchdog timeout. range: 5~60.");
    mvx_argp_add_opt(&argp, 0, "profiling", true, 1, "0", "enable profiling for bandwidth statistics . 0:disable; 1:enable.");
