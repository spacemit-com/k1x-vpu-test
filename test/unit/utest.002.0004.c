...
#DESCRIPTION: available at the last position.
*/
...
**********************************************************/
struct device;
struct mutex { struct device * placeholder; };
...
int main( int argc, char ** argv )
{
	...
	if ( instances )
	{
		...
		/* Install the fake pointer at the last position */
		n = sizeof( instances->instances ) / sizeof( instances->instances[0] );
		instances->instances[n-1] = test_ptr;

		/* We expect to find the device at position n-1
		 */
		if ( (n-1) == find_hw_dev( instances, test_ptr ) ) {
			reply = 0;
		}
		free( instances );
	}
