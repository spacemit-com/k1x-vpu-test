...
public:
    ...
    void setFd(const int fd, const unsigned int plane = 0, const unsigned offset = 0);
...
public:
    ...
    void setFormat(uint32_t pixelformat, unsigned int numPlanes = 0, unsigned int width = 0, unsigned int height = 0);
...
BufferV4L2::BufferV4L2(const int &fd, const v4l2_buf_type type, const unsigned int index) :
    fd(fd),
    queued(false)
{
    buf.type = type;
...
const v4l2_buffer &BufferV4L2::getBuffer()
{
    return buf;
}
...
void BufferV4L2::checkPlanes(unsigned int plane) const
{
    ...
    {
        throw Exception("Failed to get buffer length. Plane out of range. plane=%u.", plane);
    }
}
...
void BufferV4L2::setLength(const unsigned int length, const unsigned int plane)
{
    checkPlanes(plane);
...
void BufferV4L2::setBytesUsed(const unsigned int bytesused, const unsigned int plane)
{
    checkPlanes(plane);
...
void BufferV4L2::setEOS()
{
    buf.flags |= V4L2_BUF_FLAG_LAST;
}

bool BufferV4L2::isQueued() const
{
    return queued;
}
...
void BufferV4L2::query()
{
    ...
    if (ret != 0)
    {
        throw Exception("Failed to query buffer. ret=%d, type=%d, index=%u.", ret, buf.type, buf.index);
    }
...
void BufferV4L2::queue()
{
    ...
    if (ret != 0)
    {
        throw Exception("Failed to queue buffer. ret=%d, type=%d, index=%u.", ret, buf.type, buf.index);
    }
...
void BufferV4L2::print(const char *prefix)
{
    ...
    if (V4L2_TYPE_IS_MULTIPLANAR(buf.type))
    {
        ...
        for (unsigned int i = 0; i < buf.length; ++i)
        {
            ...
            cout << "{bytesused=" << plane.bytesused <<
            ", length=" << plane.length <<
...
int BufferV4L2::getFd(const unsigned int plane) const
{
    ...
    {
        throw Exception("Failed to get file descriptor. Illegal memory type. memory=%d.", buf.memory);
    }
...
void BufferV4L2::setFd(int fd, const unsigned int plane, const unsigned offset)
{
    checkPlanes(plane);
...
void Port::setFormat(uint32_t pixelformat, unsigned int numPlanes, unsigned int width, unsigned int height)
{
...
{
    v4l2_requestbuffers reqbuf = { .count = count, .type = format.type, .memory = memory };
    ...
    for (size_t i = buffers.size(); i < reqbuf.count; ++i)
    {
        ...
        if (memory == V4L2_MEMORY_DMABUF)
        {
            for (unsigned int plane = 0; plane < buffer->getNumPlanes(); ++plane)
            {
                ...
                BufferION::BufferType bufferType = V4L2_TYPE_IS_OUTPUT(format.type) ? BufferION::BUFFER_TYPE_INPUT : BufferION::BUFFER_TYPE_OUTPUT;
...
BufferV4L2 *Port::dequeueBuffer()
{
    ...
    if (ret != 0)
    {
        throw Exception("Failed to dequeue buffer. ret=%d, type=%d.", ret, buf.type);
    }
...
BufferV4L2 *Port::getBuffer(unsigned int index)
{
    return buffers.at(index);
}

size_t Port::getBufferCount() const
{
    return buffers.size();
}
...
Codec::Codec(const v4l2_buf_type input, const v4l2_buf_type output) :
    fd(-1)
{
...
void Codec::setSecureVideo(bool securevideo)
{
    struct v4l2_control ctrl = {
        .id = V4L2_CID_MVE_VIDEO_SECURE_VIDEO, .value = securevideo
    };
...
int main(int argc, char **argv)
{
    ...
    while (true)
    {
        ...
        if (p.revents & POLLIN)
        {
            ...
            if (buffer->getBytesUsed() == 0 && (buf.flags & V4L2_BUF_FLAG_ERROR) == 0)
            {
