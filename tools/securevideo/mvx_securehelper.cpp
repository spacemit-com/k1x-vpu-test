...
static void helpFirmware(const char *exe)
{
    ...
    cerr << "    DST     Destination where to write the firmware descriptor." << endl;
}
...
static void helpMemory(const char *exe)
{
    ...
    cerr << "    DST     Destination where to write the memory descriptor." << endl;
}
...
static int allocateMemory(const char *exe, int argc, char **argv)
{
    ...
    {
        Session session;
        BufferION *buffer = session.allocateBuffer(BufferION::BUFFER_TYPE_FIRMWARE, size);
