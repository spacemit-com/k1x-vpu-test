...
public:
    ...
    BufferION(const BufferType type, const std::size_t size, TEEC_Context &context);
    ~BufferION();
...
{
public:
    Firmware(const char *file, const unsigned int ncores, TEEC_Context &context, TEEC_Session &session);
    ~Firmware();
...
public:
    ...
    BufferION *allocateBuffer(const BufferION::BufferType type, const std::size_t size);
