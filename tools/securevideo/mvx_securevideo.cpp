...
Session::Session()
{
    ...
    uint32_t err;
    res = TEEC_OpenSession(&context, &session, &uuid, TEEC_LOGIN_PUBLIC, NULL, NULL, &err);
    if (res != TEEC_SUCCESS)
    {
...
Firmware *Session::loadFirmware(const char *fw, unsigned int cores)
{
    ...
    const FirmwareSecureDescriptor &firmwareDescriptor = firmware->getFirmwareDescriptor();
...
BufferION *Session::allocateBuffer(const BufferION::BufferType type, const std::size_t size)
{
...
Firmware::Firmware(const char *file, const unsigned int ncores, TEEC_Context &context, TEEC_Session &session) :
    ncores(ncores),
    context(context),
    session(session),
    secureBuffer(0)
{
    ...
    secureBuffer = new BufferION(BufferION::BUFFER_TYPE_FIRMWARE, SIZE_4M, context);
...
{
    TEEC_Operation op = { 0 };
    op.paramTypes = TEEC_PARAM_TYPES(TEEC_MEMREF_TEMP_INPUT, TEEC_MEMREF_PARTIAL_OUTPUT, TEEC_MEMREF_TEMP_OUTPUT, TEEC_VALUE_INPUT);
    ...
    uint32_t err;
    TEEC_Result result = TEEC_InvokeCommand(&session, SEDGET_VIDEO_TA_CMD_LOAD_FW, &op, &err);
    if (result != TEEC_SUCCESS)
    {
        throw Exception("TA Load firmware failed. result=0x%x, error=%u.", result, err);
    }
}
...
BufferION::BufferION(const BufferType type, const size_t size, TEEC_Context &context) :
    context(context),
    size(size),
    fd(-1)
{
...
size_t BufferION::getSize() const
{
    return size;
}

int BufferION::getFileDescriptor() const
{
    return fd;
}

TEEC_SharedMemory &BufferION::getSharedMemory()
{
    return sharedMemory;
}
...
void BufferION::registerSharedMemory()
{
    ...
    TEEC_Result result = TEEC_RegisterSharedMemoryFileDescriptor(&context, &sharedMemory, fd);
    if (result != TEEC_SUCCESS)
    {
        throw Exception("TEEC_RegisterMemoryFileDescriptor failed. result=0x%x.", result);
    }
}
